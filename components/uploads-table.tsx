...
export function UploadsTable() {
  ...
  let filteredUploads = uploads.filter((upload) => {
    const nameMatch = upload.file_name.toLowerCase().includes(lowerSearch)
    const captionMatch = upload.caption?.toLowerCase().includes(lowerSearch)
    const uploader = upload.editors?.name ?? ""
    const uploaderMatch = uploader.toLowerCase().includes(lowerSearch)
    return nameMatch || !!captionMatch || uploaderMatch
  })
              ...
              <tbody>
                {filteredUploads.map((upload) => (
                  <tr key={upload.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <p className="text-gray-900">{upload.editors?.name ?? "Unknown"}</p>
                    </td>
                    <td className="py-3 px-4">
