...
export function LogsViewer() {
  ...
  const handleArchiveUploads = async () => {
    if (!confirm("Archive all uploads? This will move all uploads into the archive (logs) and remove them from Uploads.")) return
    try {
      setIsLoading(true)
      const res = await fetch("/api/admin/archive-uploads", { method: "POST" })
      const json = await res.json()
      if (!res.ok) throw new Error(json?.error || "Failed to archive uploads")
      // refresh logs after archival
      await fetchLogs()
      // Also optionally you may want to refresh other pages that show uploads; UI should reflect uploads removal
    } catch (err) {
  ...
  const handleDeleteAllArchives = async () => {
    ...
    if (!confirm(`Delete all ${logs.length} archived log entries? This is permanent.`)) return
    try {
      setIsDeleting(true)
        ...
        {isLoading ? (
          ...
          <>
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-200">
              ...
              <div className="flex items-center gap-2">
                <Button onClick={handleArchiveUploads} disabled={isLoading} className="gap-2">
                  Archive uploads
                </Button>

                <Button variant="destructive" onClick={handleDeleteAllArchives} disabled={isDeleting || logs.length === 0} className="gap-2">
                  Delete archives
                </Button>
              </div>
            </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-600 mb-1">
                      {log.editors?.name ?? "Unknown"} ({log.editors?.type === "video" ? "Video Editor" : "Graphic Designer"})
                    </p>
                    {log.caption && (
