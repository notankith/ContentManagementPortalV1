...
export function UploadForm({ editorId, editorName, editorType, onUploadSuccess }: UploadFormProps) {
      ...
      xhr.onload = () => {
        try {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            // Handler returns { url, path }
            resolve({ url: json.url ?? json.rawUrl ?? json.raw_url ?? "", path: json.path })
          } else {
            let msg = "Upload failed"
            try {
              const json = JSON.parse(xhr.responseText)
              msg = json?.error || json?.message || xhr.statusText
            } catch (e) {
  ...
  let uploadedThumbnailPath: string | null = null
      ...
      if (editorType === "video" && thumbnail) {
        try {
          const res = await uploadBlob(thumbnail, {
            mediaType: "image",
            isThumbnail: true,
          })
          uploadedThumbnailUrl = res.url || null
          uploadedThumbnailPath = res.path || null
        } catch (thumbError) {
      ...
      const blobUrl = typeof uploadResult === "string" ? uploadResult : uploadResult.url
      const blobPath = typeof uploadResult === "object" ? uploadResult.path : undefined
